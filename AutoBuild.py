...
# 导出xcarchive,ipa
def build_project(conf,bundleID,plistPath):
    ...
    # 导出xcarchive
    build = 'xcodebuild -workspace %s -scheme %s -configuration %s -archivePath %s clean archive build PRODUCT_BUNDLE_IDENTIFIER="%s"' %(xcworkPath,conf['project_name'],conf['configuration'],xcarchivePath,bundleID)
    print(build);
    os.system(build)
...
def get_build_project_data():
    ...
    conf = {'project_path':cf.get('conf', 'project_path'),
            'project_name':cf.get('conf', 'Project_Name'),
    ...
    if index < 4:
        ...
        build_project(conf,cf.get(key,'BundleID'),cf.get(key,'PlistPath'))
    else:
        print("无效序号!")
